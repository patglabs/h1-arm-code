...
def main():
    pygame.init()
    # Increased window size to accommodate 1.5x larger font sizes
    screen = pygame.display.set_mode((1400, 900))
    pygame.display.set_caption("Terminal de Contrôle du Bras Robotique")
    
    # Fonts are 1.5x larger than previous (72 * 1.5 = 108, 48 * 1.5 = 72)
    ...
    while running:
        ...
        if app_state == "MENU":
            ...
            title = font_large.render("Sélectionnez le Port Série", True, (255, 255, 255))
            screen.blit(title, (20, 20))

            # Draw Refresh Button
            refresh_rect = pygame.Rect(1080, 20, 280, 70)
            pygame.draw.rect(screen, (100, 100, 150), refresh_rect, border_radius=5)
            screen.blit(font_small.render("Actualiser", True, (255,255,255)), (1100, 30))
            ...
            if not available_ports:
                screen.blit(font_small.render("Aucun port trouvé. Branchez l'ESP32.", True, (200, 100, 100)), (20, y_offset))
            else:
                for port in available_ports:
                    port_rect = pygame.Rect(20, y_offset, 1300, 100)
                    color = (70, 70, 90) if port_rect.collidepoint(mouse_pos) else (50, 50, 70)
                    pygame.draw.rect(screen, color, port_rect, border_radius=5)
                    screen.blit(font_large.render(port, True, (200, 255, 200)), (40, y_offset + 20))
            ...
            screen.blit(instruction, (20, 800))
        ...
        # ---------------------------------------------------------
            ...
            if ser is None or not ser.is_open:
                status_text = font_large.render(f"DÉCONNECTÉ: {selected_port}", True, (255, 100, 100))
                sub_text = font_small.render("Tentative de reconnexion auto...", True, (200, 200, 200))
                ...
                sub_text = font_small.render("Contrôles actifs. Focus sur la fenêtre.", True, (200, 200, 200))
                
            screen.blit(status_text, (20, 20))
            screen.blit(sub_text, (20, 110))
            
            # --- Draw Key Mapping Map (Left Side) ---
            map_title = font_small.render("Assignation des Touches :", True, (255, 255, 100))
            screen.blit(map_title, (20, 220))
            ...
            for keys_text, desc_text in controls_data:
                ...
                # 2. Render Description in Gray (Adjusted X offset to 400 for larger text)
                desc_surface = font_small.render(desc_text, True, (180, 180, 180))
                screen.blit(desc_surface, (400, map_y))
                
                map_y += 75

            # --- Draw Active Keys (Right Side) ---
            active_title = font_small.render("Actuellement Actif :", True, (255, 255, 100))
            screen.blit(active_title, (850, 220))

            y_offset = 300
            if not active_keys:
                idle_text = font_small.render("En attente...", True, (100, 100, 100))
                screen.blit(idle_text, (850, y_offset))
            else:
                for key in active_keys:
                    action_text = font_large.render(f"{KEY_MAPPINGS[key]['name']}", True, (100, 200, 255))
                    screen.blit(action_text, (850, y_offset))
                    y_offset += 90

            instruction = font_small.render("Appuyez sur ÉCHAP pour retourner au Menu.", True, (150, 150, 150))
            screen.blit(instruction, (20, 800))
